...
where Nome_produto="Paralama" or Nome_produto="Arruela";

create view vw_data
select p.Nome_produto from Produtos p
join Itens_Solicitacao it
on p.Cod_Produto = it.Cod_Produto
join Solicitacoes s
on it.Num_Sol = s.Num_Sol
where Data_sol like "%2018-02%";

select fu.Nome_Func, sum(it.Valor) From Funcionarios fu
join Solicitacoes sol
on fu.Cod_Func = sol.Cod_Func
join Itens_Solicitacao it
on sol.Num_Sol = it.Num_Sol
where fu.Cod_Func=300;

select * from vw_depart;
